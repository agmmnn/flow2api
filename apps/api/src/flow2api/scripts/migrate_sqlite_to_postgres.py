...
Commands are intentionally explicit and restartable:
    ...
    python -m flow2api.scripts.migrate_sqlite_to_postgres preflight
    python -m flow2api.scripts.migrate_sqlite_to_postgres backfill
    python -m flow2api.scripts.migrate_sqlite_to_postgres cutover --confirm CUTOVER
    python -m flow2api.scripts.migrate_sqlite_to_postgres verify
    python -m flow2api.scripts.migrate_sqlite_to_postgres abort --confirm ABORT
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--sqlite", type=Path, default=runtime_data_dir / "flow.db")
    ...
    parser.add_argument("--state-dir", type=Path, default=runtime_data_dir / "migration" / "postgres-bridge")
