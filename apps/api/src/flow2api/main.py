...
def _api_only_hostnames() -> set[str]:
    """Comma-separated FQDNs from ``FLOW2API_API_ONLY_HOST`` (see ``infra/compose``)."""
...
# Create FastAPI app
app = FastAPI(
    title="Flow2API", description="OpenAI-compatible API for Google VideoFX (Veo)", version="1.0.0", lifespan=lifespan
)
...
async def serve_spa(full_path: str, request: Request):
    ...
    return HTMLResponse(
        content="<h1>Flow2API GUI</h1><p>Frontend not found. Please build the frontend first.</p>", status_code=404
    )
